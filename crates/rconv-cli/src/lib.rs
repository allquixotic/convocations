...
/// Check if the provided arguments are complete enough for CLI-only execution
pub fn should_run_cli_mode() -> bool {
    // Check if we have any arguments beyond the binary name
    let args: Vec<String> = std::env::args().collect();
    if args.len() <= 1 {
        return false; // No args = GUI mode
    }

    // Try to parse with clap
    // If parsing succeeds and we have a command or sufficient process args, run CLI mode
    match Cli::try_parse() {
        Ok(cli) => {
            // If there's a subcommand (preset, secret), run CLI mode
            if cli.command.is_some() {
                return true;
            }
            // If process args are not empty, run CLI mode
            !cli.process.is_empty()
        }
        Err(_) => {
            // If parsing fails, it might be --help or invalid args
            // Let clap handle --help and --version in CLI mode
